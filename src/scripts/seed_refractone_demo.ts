...
const sources = [
  ...
  {
    domain: "refractone.com",
    url: "https://refractone.com/",
    title: "Refractone AI visibility platform",
    source_type: "YOU",
    url_type: "HOMEPAGE",
  },
  {
    domain: "refractone.com",
    url: "https://refractone.com/ai-visibility-tools",
...
function modelAnswer(engine: string, prompt: string, ownMentioned: boolean, mentionedBrands: string[]) {
  ...
  const ownLine = ownMentioned
    ? "Refractone is a strong fit for lean B2B teams that want visibility tracking, source intelligence, competitor comparisons, and Sara-style recommendations without enterprise complexity."
    : "Refractone is relevant to this category, but it is not cited as consistently as the larger AI visibility platforms in current answer sets."
...
async function seedWebAnalytics(projectId: string) {
  const site = await prisma.webAnalyticsSite.create({
    data: {
      ...
      name: "Refractone Website",
      domain: "refractone.com",
      public_key: `demo_${Date.now()}_refractone`,
      is_active: true,
    },
  })
    ...
    for (let index = 0; index < sessionsForDay; index += 1) {
      ...
      await prisma.webAnalyticsEvent.create({
        data: {
          ...
          url: `https://refractone.com${session.landing_page ?? "/"}`,
          title: "Refractone - AI Visibility Platform",
  ...
  await prisma.saraMessage.createMany({
    data: [
      ...
      },
      {
        ...
        role: "ASSISTANT",
        content:
          "Refractone is appearing in 64% of tracked AI answers this week, with the strongest visibility in GEO strategy and AI visibility tool prompts. The biggest gap is competitor comparison prompts where Peec AI and PromptWatch are cited more often from third-party sources.",
        citations: [
        ...
        suggested_actions: [
          ...
          "Refresh source citations on Refractone.com",
...
async function main() {
      ...
      for (const [engineIndex, engine] of engines.entries()) {
        ...
        const chat = await prisma.chat.create({
          data: {
            ...
            screenshot_path: `/demo/refractone/${run.id}-${promptIndex}-${engine}.png`,
  ...
  await prisma.helpCenter.create({
    data: {
      ...
      message: "Demo ticket: I want to understand why PromptWatch is ranking above Refractone in comparison prompts.",
      is_resolved: false,
    },
  })

  console.log("Refractone demo seed complete")
